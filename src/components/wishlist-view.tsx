...
export function WishlistView() {
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-xl border border-teal-900/10 bg-teal-50/40 p-4 sm:p-5">
        <AddLinkForm />
      </div>
      ...
      {state.products.length === 0 ? (
        ...
        <ul className="space-y-3">
          {state.products.map((product) => {
            ...
            const best = activeOffers.reduce<
              (typeof activeOffers)[number] | null
            >(
              (acc, o) => (!acc || o.lastPrice < acc.lastPrice ? o : acc),
              null,
            );
            const fromLink = product.queryText.startsWith("http");
            return (
              <li
                key={product.id}
              ...
              >
                  ...
                  <div className="min-w-0 space-y-1">
                    ...
                    {best ? (
                      <p className="text-sm text-teal-900">
                        Best tracked:{" "}
                        {formatMoney(best.lastPrice, best.currency)} on{" "}
                        {best.merchant}
                      </p>
                    ) : (
                ...
                <div className="flex shrink-0 items-center gap-1 self-end sm:self-center">
                  <Button
                    ...
                    onClick={() =>
                      setNotifyEnabled(product.id, !product.notifyEnabled)
                    }
                  >
                    {product.notifyEnabled ? (
